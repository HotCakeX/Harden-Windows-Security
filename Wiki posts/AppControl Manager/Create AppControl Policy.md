...
* **App ID Tagging Policy**: When enabled, the policy that will be created will be an App ID Tagging policy type based on the selected template. It will have the same rules as the template you selected. It's only available for `Allow Microsoft` and `Default Windows` templates. The created App ID Tagging policies will each have one default tag in them that are `AllowMSFTTagKey` and `AllowMSFTTagKey` respectively with the value of `True`.
