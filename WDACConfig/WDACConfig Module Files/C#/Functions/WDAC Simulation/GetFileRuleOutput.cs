...
namespace WDACConfig
{
    public static class GetFileRuleOutput
    {
        ...
        public static HashSet<WDACConfig.PolicyHashObj> Get(XmlDocument xml)
        {
                ...
                foreach (XmlNode fileRule in fileRules)
                {
                    if (fileRule.Attributes != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(friendlyName))
                        {
                            ...
                            var hashTypeMatch = System.Text.RegularExpressions.Regex.Match(friendlyName, @".* (Hash (Sha1|Sha256|Page Sha1|Page Sha256|Authenticode SIP Sha256))$");
                            ...
                            var filePathForHash = System.Text.RegularExpressions.Regex.Replace(friendlyName, @" (Hash (Sha1|Sha256|Page Sha1|Page Sha256|Authenticode SIP Sha256))$", string.Empty);
                            ...
                            {
                                outputHashInfoProcessing.Add(new WDACConfig.PolicyHashObj(hashValue, hashType, filePathForHash));
                            }
                        }
                    }
                }
            ...
            outputHashInfoProcessing = new HashSet<WDACConfig.PolicyHashObj>(outputHashInfoProcessing.Where(obj => obj.HashType == "Hash Sha256"));

            WDACConfig.VerboseLogger.Write($"Returning {outputHashInfoProcessing.Count} file rules that are based on file hashes");
