...
function Confirm-SystemCompliance {   
    ...
    begin {
        ...
        # Import the CSV file
        $csv = Import-Csv -Path "C:\Users\HotCakeX\OneDrive\Desktop\Compliance Checking processing part 2\Registry Checks.csv"

        # An object to hold all initial registry items
        $AllRegistryItems = @()

        # Loop through each row in the CSV file
        foreach ($row in $csv) {
            $AllRegistryItems += [PSCustomObject]@{
                # Get the category, key, name, value, and type from the row
                category = $row.Category
                key      = $row.Key
                name     = $row.Name
                value    = $row.Value
                type     = $row.Type
                # Build the registry path from the $key
                regPath  = "Registry::$($row.Key)"
            }
        ...
        function Invoke-CategoryProcessing {
            param($catname, $Method)
        
            $output = @()
        
            foreach ($item in $AllRegistryItems | Where-Object { $_.category -eq $catname }) {
                ...
                try {
                    $regKey = Get-Item -Path $item.regPath -ErrorAction Stop
                    # If no error is thrown, the key exists
                    $keyExists = $true
            
                    # Try to get the registry value and type
                    try {
                        $regValue = Get-ItemPropertyValue -Path $item.regPath -Name $item.name -ErrorAction Stop
    ...
    process {
        ...
        $NestedObjectArray += [PSCustomObject](Invoke-CategoryProcessing -catname "Microsoft Defender" -Method "Group Policy")
